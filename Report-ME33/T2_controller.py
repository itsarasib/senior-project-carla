...
import sys
sys.path.append('D:/carla/PythonAPI/carla')  # Adjust the path as necessary


from carla.agents.navigation.constant_velocity_agent import ConstantVelocityAgent
from carla.agents.tools import misc as tool
...
Decision = True
Velocity = 20
spawn_point = [waypoint for waypoint in waypoints if waypoint.road_id == 8][-17].transform
destination = [waypoint for waypoint in waypoints if waypoint.road_id == 741][5].transform.location
predestination = [waypoint for waypoint in waypoints if waypoint.road_id == 45][-5].transform.location
...
# Car_Data_Format = [TTC1[s],TTC2[s],TTC3[s],ID[int],TYPE[int],S[m],V[km/hr]]
...
def T2D():#Dicision Making System
    ...
    if (Car_Data[T2I][5][-1] > 0) and (Car_Data[T2I][6][-1] > V_cornering):
        ...
        if T2S == 2:
            for i in Car_Data:
                if i[4] != 0:
                    if ((i[5][-1] > -LB) and (i[5][-1] <= 0)) or ((i[0] < CFA) and (i[0] > 0)):
                        return [0,6]
            return [2*a,7]
        #T2 Full Braking
        elif T2S == 1:
...
def T2Plot():
    ...
    T2[0,0].grid(True)
    T2[0,1].plot(Car_Data[[j[4] for j in Car_Data].index(0)][8], Car_Data[[j[4] for j in Car_Data].index(0)][6],T2_Carla[0],T2_Carla[1])
...
def VSPlot():
    fix, ax = plt.subplots()
    twin1 = ax.twinx()
    p1, = ax.plot(Car_Data[[j[4] for j in Car_Data].index(0)][8], Car_Data[[j[4] for j in Car_Data].index(0)][5], "C0", label="Distance")
    p2, = twin1.plot(Car_Data[[j[4] for j in Car_Data].index(0)][8], Car_Data[[j[4] for j in Car_Data].index(0)][6], "C1", label="Velocity")
...
def game_loop(args):
    global tt, Car_Data, Car_ID, t, Velocity, Keyword
    """
